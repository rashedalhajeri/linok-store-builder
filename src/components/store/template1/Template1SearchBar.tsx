...
interface Template1SearchBarProps {
  onSearch: (query: string) => void;
}

export const Template1SearchBar = ({ onSearch }: Template1SearchBarProps) => {
  const [showFilters, setShowFilters] = useState(false);

  return (
    <div className="w-full max-w-xl mx-auto space-y-4">
      <div className="relative w-full">
        <Search className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[#ee9ca7]/60" />
        <Input
          type="text"
          placeholder="ابحث عن إعلان..."
          className="pr-12 py-6 text-lg rounded-2xl shadow-md border border-gray-100 bg-white/95 backdrop-blur-sm
                   hover:bg-white transition-all duration-300 focus:ring-2 focus:ring-[#ee9ca7]/20
        ...
        <Button
          variant="ghost"
          size="icon"
          className="absolute left-3 top-1/2 -translate-y-1/2 hover:bg-gray-50"
